...
if ~isequal(numel(imgs_Cy5),numel(imgs_GFP),numel(imgs_RFP))
...
for id = 1:numel(imgs_GFP)
    fprintf('%d / %d\n',id,numel(imgs_GFP));
    tiffInfo = imfinfo([path,imgs_GFP(id).name]);
    z = numel(tiffInfo);
    x = tiffInfo.Width;
    y = tiffInfo.Height;
...
%     axis equal
    ...
    for j = 1:size(Cy5_coor,1)
        Cy5_distance(j) = sqrt(min(sum((GFP_coor-Cy5_coor(j,:)).^2,2)));
    end
...
writecell({'Cy5',Cy5_dis;'RFP',RFP_dis},[save_path,'result.xlsx']);
